...
figure(1);
stem([1:length(PAM_m_modu)],PAM_m_modu);
...
QAM_m_I_phase_ch = m_I_phase_Amp + randn(1,length(m_I_phase_Amp))*sqrt(QAM_noise_power) ;
QAM_m_Q_phase_ch = m_Q_phase_Amp + randn(1,length(m_Q_phase_Amp))*sqrt(QAM_noise_power) ;
...
% de
for b = 1:(num_of_series/QAM_R)
    bb_I(b) = find(abs(QAM_desicion_I - QAM_m_I_phase_ch(b)) == min(abs(QAM_desicion_I - QAM_m_I_phase_ch(b))))-1;
    QAM_m_I_phase_demo(b,:) = QAM_graycode_I(bb_I(b)+1,:);
    bb_Q(b) = find(abs(QAM_desicion_Q - QAM_m_Q_phase_ch(b)) == min(abs(QAM_desicion_Q - QAM_m_Q_phase_ch(b))))-1;
    QAM_m_Q_phase_demo(b,:) = QAM_graycode_Q(bb_Q(b)+1,:);
end
QAM_count = sum(([QAM_m_I_phase_demo QAM_m_Q_phase_demo]-[QAM_m_I_phase_bin QAM_m_Q_phase_bin])');
QAM_Error_Symbol = 1-sum(1.*(QAM_count == 0))/(num_of_series/QAM_R);
...
% certification
QAM_test_SNR = log10(mean(m_I_phase_Amp.^2+m_Q_phase_Amp.^2)/mean((randn(1,length(PAM_m_modu))*sqrt(QAM_noise_power)).^2)*2)*10;
