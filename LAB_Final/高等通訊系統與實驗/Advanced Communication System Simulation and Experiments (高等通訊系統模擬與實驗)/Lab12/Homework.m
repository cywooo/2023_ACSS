...
%delay = (length(SRRC_filter(trun,4,alpha))-1)/2;
...
noise = sqrt(noise_power)* randn(1,length(dma_sym));
received_signal = dma_sym + noise;

%% DMA (Receiver)
dma_receive = filter(DMA,received_signal);
dma_receive = dma_receive(group_delay+1:end);
% alpha = 0.3;
% dma_receive = conv(received_signal,SRRC_filter(trun,4,alpha),'same');
%delay = (length(SRRC_filter(trun,4,alpha))-1)/2;
%dma_receive = dma_receive(delay+1:end-delay);
...
% figure(2);
% test = Gaussian_filter(20,M,trun);
% plot(10*log10(test));
% hold on
% x = 0:160;
% plot(x,-3*ones(size(x)));
