...
fd_list = 150*10.^((1:7)-1);
BER = zeros(1,length(fd_list));
iteration = 1000;

for s=1:length(fd_list)
    BER_temp = 0;
...
%% Plot
figure(1);
semilogx(2*Tb*fd_list,BER,'--o');
xlabel('Modulation Index (h=2fdTb)');
ylabel('BER');
title('BER with different modulation index');
