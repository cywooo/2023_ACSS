...
plot(rect);
hold on
plot(real(downsample_s)*0.3);
figure(2);
plot(tri);
hold on
plot(imag(downsample_s)*0.2);

% Verify
alp = (1+g*exp(i*phi))/2;
beta = (1-g*exp(i*phi))/2;
veri_s = alp*s + beta*conj(s);
figure(3);
plot(real(downsample_s)*0.3);
hold on
plot(real(veri_s));
figure(4);
plot(imag(downsample_s)*0.3);
hold on
plot(imag(veri_s));
figure(5);
plot(downsample_s*0.3);
hold on
plot(veri_s);
