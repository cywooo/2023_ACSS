...
yi = sqrt(2)*x.*cos(2*pi*(fc+CFO)*(1:size(r,2))  );
yq = (-1)*sqrt(2)*x.*sin(2*pi*(fc+CFO)*(1:size(r,2))  );
...
% figure(2)
% subplot(4,1,1);stem(fftshift(abs(fft(yi))));title('Y_I freq')
% subplot(4,1,2);stem(fftshift(abs(fft(yif))));title('Filter Y_I freq')
% subplot(4,1,3);stem(fftshift(abs(fft(yq))));title('Y_Q freq r')
% subplot(4,1,4);stem(fftshift(abs(fft(yqf))));title('Filter Y_Q freq')
