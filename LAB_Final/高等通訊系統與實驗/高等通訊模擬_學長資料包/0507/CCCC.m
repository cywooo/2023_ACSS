...
%xe = x.*exp(-1*1i *(2*pi*(fc)*(1:size(ffupup,2)) + phase) );
...
subplot(4,1,2);stem(fftshift(abs(fft(xe))));title('先調到中頻')
subplot(4,1,3);stem(fftshift(abs(fft(xef))));title('在中頻濾掉')
subplot(4,1,4);stem(fftshift(abs(fft(XX))));title('繼續降頻')


delay =0;
figure(3)
stem([BPSK(1:end-delay)' 5000*xefdfd(1+delay:end)']);
