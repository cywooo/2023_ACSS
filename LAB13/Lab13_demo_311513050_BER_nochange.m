...
%% 
        ...
        signal_length = 1e4 + 10; % signal length adjust
        % signal generate------
...
for q = [1:length(SNR)]
        ...
        OSNR_ZF_dB(q) = 10*log10(OSNR_ZF);
        OSNR_MMSE = 1/(sum(abs(s_11_MMSE_reDig_normalized([6:end-5])-s_2([6:end-5])).^2)/(signal_length-10));
        OSNR_MMSE_dB(q) = 10*log10(OSNR_MMSE);

        BER_ZF(q) = sum(abs(sign(real(s_11_ZF_reDig([6:end-5])))-s_2([6:end-5])))/2/(signal_length-10);
        BER_MMSE(q) = sum(abs(sign(real(s_11_MMSE_reDig([6:end-5])))-s_2([6:end-5])))/2/(signal_length-10); 
end
