...
carrier_rotate = 2*pi*30/360*DMA_rate;
carrier_rece = exp(1i.*(2*pi.*fc+carrier_rotate).*[1:length(x_ideal)]/DMA_rate);

s_06_rece_ideal = x_ideal ./ carrier_rece;
s_07_DMA = filter(Lab10_demo_IIR,s_06_rece_ideal);      
...
figure;
scatter(real(s_08_reDig_p1_normalized),imag(s_08_reDig_p1_normalized));
title_text = "compensate Transmit & Receive(imag), practice 2 g = "+num2str(imbalance_g)+", phi = "+num2str(imbalance_phi);
title(title_text,"fontsize",12);
