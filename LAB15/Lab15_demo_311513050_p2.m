...
Spectrum_mask_y = -66.*(Spectrum_mask_x<(IF_freq-2.5e6))+...
                  -46.*(Spectrum_mask_x>=(IF_freq-2.5e6)).*((Spectrum_mask_x<IF_freq-1.5e6))+...
                  ...
                  -46.*(Spectrum_mask_x>=(IF_freq+1.5e6)).*((Spectrum_mask_x<IF_freq+2.5e6))+...
                  -66.*(Spectrum_mask_x>=(IF_freq+2.5e6));
...
S_02_Gussianf = fftshift(fft(s_02_Gussianf));

figure;
plot(linspace(-1/2*DAC_rate,1/2*DAC_rate,length(S_02_Gussianf)),abs(S_02_Gussianf));
title_text = "Gussian F";
title(title_text,"fontsize",12);
...
S_03_samation = fftshift(fft(s_03_samation));
figure;
plot(linspace(-1/2*DAC_rate,1/2*DAC_rate,length(S_03_samation)),abs(S_03_samation));
title_text = "samation F";
title(title_text,"fontsize",12);

% load IF frequency-------
s_04_IF = s_03_samation .* exp(1i*2*pi*IF_freq/DAC_rate.*[1:length(s_03_samation)]);

S_04_IF = fftshift(fft(s_04_IF));
figure;
plot(linspace(-1/2*DAC_rate,1/2*DAC_rate,length(S_04_IF)),abs(S_04_IF));
title_text = "IF F";
title(title_text,"fontsize",12);

figure;
plot([1:length(s_04_IF)],real(s_04_IF));
title_text = "IF n";
title(title_text,"fontsize",12);
...
figure;
plot([1:length(sum_s_02_Gussianf)],real(sum_s_02_Gussianf));
title_text = "toChannel n";
title(title_text,"fontsize",12);
