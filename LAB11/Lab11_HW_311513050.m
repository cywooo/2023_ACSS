...
equal_bit = real(equal_bit)/sqrt(sum((real(equal_bit).*(real(equal_bit)>0.5)).^2)/sum(real(equal_bit)>0.5))+...
               imag(equal_bit)/sqrt(sum((imag(equal_bit).*(imag(equal_bit)>0.5)).^2)/sum(imag(equal_bit)>0.5))*1i;
...
stem([1:signal_length],real(s_11_IF_reDig_split_normalized(3,:)),"rd");
hold off;
legend("re","path 1","path 2","path 3");
...
stem([1:signal_length],imag(s_11_IF_reDig_split_normalized(3,:)),"rd");
hold off;
legend("re","path 1","path 2","path 3");
...
figure(4);
stem([-length(equizer)/2:length(equizer)/2-1],equizer,"bo");
...
figure(3);
subplot(2,1,1);
stem([1:length(equal_bit)]-equalizer_len/2,real(equal_bit),"bo");
hold on;
stem([1:signal_length],s_real,"r+");
hold off;
legend("re_e_q","tr");
...
title(title_text,"fontsize",12);
subplot(2,1,2);
stem([1:length(equal_bit)]-equalizer_len/2,imag(equal_bit),"bo");
hold on;
stem([1:signal_length],s_imag,"r+");
hold off;
legend("re_eq","tr");
